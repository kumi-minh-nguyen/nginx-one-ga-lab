---
title: NGINX One Console Lab - Part 5
description: Managing NGINX ONE Config Sync Group with NGINX Declarative API
order: 5
---
...
## How to configure DAPI to talk to NGINX ONE?

- Add NGINX Instances to a config sync group
- Generate API Token for DAPI
- Input values for DAPI API request
...
1. Add NGINX instance into Config Sync Group
    ...
    - Go to **Lab components**, look for **NGINX Expansion** VM, access it with **Web Shell** method.
    - Change the VM hostname to make it unique in the shared tenant: `hostnamectl set-hostname` **<GetVariable name="petname" />-nginx-expansion**.
...
1. Input values for DAPI API request
    ...
    - On the right hand side, choose **Variables** tab, fill in the below fields for **Current value** column
        - **ncg_host**: URL without *https://* from VM Lab Extension **Postman** access method
![Postman Access Method](media/part5-02.png)
        - **ncg_port**: 443
        - **nginx_license_token** : If you're running NGINX R33, this is the NGINX JWT license token. Use this JWT for this lab:

    *eyJhbGciOiJSUzUxMiIsInR5cCI6IkpXVCIsImtpZCI6InYxIiwiamt1IjoiaHR0cHM6Ly9wcm9kdWN0LmFwaXMuZjUuY29tL2VlL3YxL2tleXMvandrcyJ9.eyJzdWIiOiJGTkktOGRiOWMwN2YtNTYxMy00NDJmLWFlYWYtZDJmNDBjMDYwNDY1IiwiaWF0IjoxNzQwMzg0MjQ5LCJpc3MiOiJGNSBJbmMuIiwiYXVkIjoidXJuOmY1OnRlZW0iLCJqdGkiOiJlZDYyNGZiMC1mMjg1LTExZWYtYTViNS0zZjVjMTdjZDA2NmUiLCJmNV9vcmRlcl90eXBlIjoiZXZhbCIsImY1X3NhdCI6MTc0Mjk3NjI0OX0.eFgjxDvSDVrE93f1cc35e-ezvhcKKu03K4rHP0m9YEbE1i6xEkaB3Yex-Out0qO2opcHP9WL33Meu19QVSo5X-tRJqoeMA0f4v37KjPR-k0GaK9yWgJFTcXtL8rsxst9QT7QGllRjgIkIW8LtgsT0EffCL98AaUKCroDIN1RwIZXMg-ra8-S-QvwQPqIpNTpMen2FGat_N0x4C8QhzoimH5x822pSUD1Vru6IarW-KzomQFQYIfgqBgw9vhsdQH6wCjNzklhQasHfbTnPPovdgpW_7VvTzOim2Yhb94e3Zw6my_2mwOeJgzOe2hF7s7E3Q31V4_-sw-G-QzT6nWAPZejPBEYC16tCmVHUfQW3jUDayGAjQyvdpIQyHK1AayZy_pEtpsPnpNSHnjv-N9pisuCKQchh-490tODkvOw4uDwB-ghlg5gjEeR1KnBj-EG9nmXqvOLL5CVR-gkr9XFJNIYB5d6GaP1SMQSOYhGFiMTOo0mExShhn5lpunbDiwehehTIt48fPY-4SKrcr7jVaJyYSgbdAUTvKOE60rT00z8GCBzAOk_boveSPGcrqVIrcXh_6M-aG_g1FlVK_lklI1wbTocWqzkdAk-sZ5uww3A0t66zM-v0o8wj3s_cENU5UwfFmPKeTqtPnLUBkt0s87OTLRsf-PbN4GJhAEFoUE*
...
1. Form the API request
    ...
    - Open the body of chosen API request, at line 27, put a `,` after the `}`, and right below line 27, insert this portion:
    ```
    "license": {
    ...
    ![postman-declaration-server-name](media/part5-03.png)
    - Under **locations**, your request should look like this:
    ![postman-location](media/part5-04.png)
    - The last part of your request should look like this:
    ![postman-last-part](media/part5-05.png)
    - You are ready to send the request to NGINX ONE!
    - If successful, you will get a response like below:
    ![postman-success](media/part5-06.png)
...
1. Test your API Gateway
    ...
    - Back to Web Shell of NGINX Expansion VM
    - Run this curl commmand `curl -k -X GET "https://apigw.nginx.lab/petstore/pet/1" | jq`
    - If you get back some data, congratulations for finishing part 5! :coffee:
