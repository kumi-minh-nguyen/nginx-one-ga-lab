...
1. Save your Data Plane Key below. This key will be used to auto-render shell commands in this lab.
...
Config Sync Group you created above. In the command below, confirm the following substitutions:
    ...
    ```bash
    curl https://agent.connect.nginx.com/nginx-agent/install | DATA_PLANE_KEY='{{NGINX_AGENT_SERVER_TOKEN}}' sh -s -- -y -c {{petname}}-manual
    ```
